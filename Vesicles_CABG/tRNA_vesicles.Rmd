...
head(counts)
```
**Диаграмма Венна: транспортные РНК все (>150 каунтов)**
```{r}
...
plt <- ggVennDiagram(venn_list, label_alpha = 0) +
  scale_fill_gradient(
    low = "white",
    high = "blue",
    limits = c(1, 40),          # диапазон значений
...
print(plt)
ggsave("./pictures_tRNA_vesicles/tRNA_Venn Diagram_vesicles.tiff", plot = plt, width = 8, height = 6, dpi = 300,  bg = "white")
```
...
```
**PCA plot for all dataset after removing donor effect**
```{r}
...
plt <- pheatmap(sampleDistMatrix,
         ...
         color = colors,
        main = 'Heatmap of the distance between tRNA samples')
...
res_150 <- res_150[order(res_150$pvalue), ]

res_sig <- res_150[res_150$pvalue < 0.05, ]
res_sig
```
**MA plot**
...
**Visualisation for gene with min padj**
```{r}
...
res_16 <- res_16[order(res_16$pvalue), ]

res_sig_16 <-  as.data.frame(res_16[res_16$pvalue < 0.05, ])
res_sig_16
```
**MA plot**
Фильтрация точек с низким средним экспрессированием (по baseMean).
...
plt
ggsave("./pictures_transpl/vesicles_boxplots.tiff", plot = plt, width = 16, height = 10, dpi = 300)
```
...
**PCA plot, type 16 after removing donor effect**
```{r}
...
plt
ggsave("./pictures_tRNA_vesicles/PCA plot for type 16 after removing donor effect.tiff", plot = plt, width = 8, height = 6, dpi = 300, bg = "white")
```
...
plt
ggsave("./pictures_tRNA_vesicles/Plot of the distance between samples_type16.tiff", plot = plt, width = 8, height = 6, dpi = 300, bg = "white")
```