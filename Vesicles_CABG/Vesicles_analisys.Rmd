...
#write.csv(anno, "anno_vesicles.csv", row.names = FALSE)
anno
```
...
coldata_new <- coldata %>%
  mutate(condition_type = paste(condition, type, sep = "_")) 
  
coldata_new <- coldata_new[, c("sample", "condition_type")]

tmp <- left_join(counts_venn, coldata_new, by = c("sample_count" = "sample"))

summed_venn <- tmp %>%
  group_by(condition_type) %>%
  summarise(across(where(is.numeric), sum)) %>%
  pivot_longer(cols = -condition_type, names_to = "miRNA", values_to = "counts") %>%
  filter(counts > 150) 

venn_a150 <- summed_venn[summed_venn$condition_type == "after_150", ]$miRNA
venn_b150 <- summed_venn[summed_venn$condition_type == "before_150", ]$miRNA
venn_a16 <- summed_venn[summed_venn$condition_type == "after_16", ]$miRNA
venn_b16 <- summed_venn[summed_venn$condition_type == "before_16", ]$miRNA
...
print(plt)
ggsave("./pictures/Venn Diagram_vesicles.tiff", plot = plt, width = 8, height = 6, dpi = 300,  bg = "white")
```
**Уникальные микроРНК для каждой группы**
```{r}
unique_b150 <- setdiff(venn_b150, c(venn_a150, venn_b16, venn_a16))
unique_a150 <- setdiff(venn_a150, c(venn_b150, venn_b16, venn_a16))
unique_b16 <- setdiff(venn_b16, c(venn_b150, venn_a150, venn_a16))
unique_a16 <- setdiff(venn_a16, c(venn_b150, venn_a150, venn_b16))

list(
  before_150 = unique_b150,
  after_150 = unique_a150,
  before_16 = unique_b16,
  after_16 = unique_a16
)
```
**Распределение количества ридов различных типов РНК по образцам**
```{r}
anno_coldata <- left_join(anno, coldata, by = c("Sample.name.s." = "sample"))
anno_coldata$Sample.name.s. <- factor(
  anno_coldata$Sample.name.s.,
  levels = anno_coldata$Sample.name.s.[order(anno_coldata$type)]
)
anno_coldata$group <- paste0(coldata$condition, "_type", coldata$type)

anno_long <- anno_coldata %>%
  pivot_longer(
    cols = where(is.numeric),
    names_to = "RNA_Type",
    values_to = "Count"
  ) %>%          
  group_by(Sample.name.s.) %>%                    
  mutate(
...
anno_long$Sample.name.s. <- factor(
  anno_long$Sample.name.s.,
  levels = c(
    "29.1p16_S39_R1_001", "15.1p16_S33_R1_001", "17.1p16_S35_R1_001",
    "29.7p16_S41_R1_001", "15.7p16_S31_R1_001", "17.7p16_S37_R1_001",
    "29.1p150_S40_R1_001", "15.1p150_S34_R1_001", "17.1p150_S36_R1_001",
    "29.7p150_S42_R1_001", "15.7p150_S32_R1_001", "17.7p150_S38_R1_001"
  )
)

plt_count <- ggplot(anno_long, aes(x = Sample.name.s., y = Count, fill = RNA_Type)) +
  geom_col() +
  theme_minimal() +
...
plt_prop <- ggplot(anno_long, aes(x = Sample.name.s., y = Proportion, fill = RNA_Type)) +
...
summed_long <- summed_anno %>%
   rowwise() %>%
   mutate(across(-group, ~ . / sum(c_across(-group)))) %>% 
   ungroup() %>%
   pivot_longer(cols = -group, names_to = "RNA_Type", values_to = "Proportion")
...
plt
ggsave("./pictures/vesicles_barplot_alldataset_normalised_by groups.tiff", plot = plt, width = 8, height = 6, dpi = 300, bg = "white")
```
**Подсчет количества ридов**
```{r}
reads_before <- anno_long %>%
  group_by(group) %>%        
  summarise(total_reads = sum(Count))
reads_before
```

## Визуализация данных: весь датасет ##
```{r}
...
plt <- pheatmap(sampleDistMatrix,
         ...
         color = colors,
         main = 'Heatmap of the distance between microRNA samples')
...
**Значимые результаты**
```{r}
signres_150 <- results(dds_150, contrast=c("condition", "after", "before"), alpha=0.05) 
summary(signres_150)
```
Let's arranged it by log2FoldChange:
```{r}
order_indices <- order(res_150$padj)
...
```
```{r}
gene_idx <- which.min(res_150$padj)
gene_name <- rownames(res_150)[gene_idx]  
...
count_data <- plotCounts(dds_150, gene=gene_idx, intgroup="condition", returnData=TRUE)
...
plt
ggsave("./pictures/boxplots_humoral_hsa-miR-208b-3p.tiff", plot = plt, width = 16, height = 10, dpi = 300)
```
...
rownames(down_150)
```
почти все риды (кроме двух) относились к hsa-miR-133a-3p 
```{r}
mirna_names_up<- c("hsa-miR-133a-3p", "hsa-miR-208b-3p", "hsa-miR-499a-5p")
mirna_names_down <- c("hsa-miR-328-3p")
```
...
Gene Ratio — это отношение количества генов из нашего набора, попавших в конкретный GO-термин, к общему числу генов в нашем анализе.
То есть если 0.01, то есть генов в GO-термине намного меньше чем в нашем списке
Чем больше список генов, тем меньше значения
```{r} 
library(scales)
...
```
```{r}
GO_enrich_up150_bp@result$Description[1:20]
```
**“RNA splicing”, “mRNA splicing, via spliceosome”, “alternative mRNA splicing, via spliceosome”, “RNA splicing, via transesterification reactions”**
	•	Операция вызывает стрессовую реакцию в клетках миокарда и сосудов, приводя к изменениям в экспрессии генов.
	•	Сплайсинг мРНК активно перестраивается в условиях гипоксии, реперфузии и воспаления, изменяя профили изоформ белков, связанных с адаптацией к повреждению.
	•	Возможна роль специфических малых РНК, включая tRFs, в модуляции сплайсинга.

**“cytoplasmic translation”, “protein-RNA complex assembly”, “ribonucleoprotein complex biogenesis”**
	•	Острая ишемия и последующая реперфузия требуют быстрого восстановления белкового синтеза, что может активировать механизмы трансляции.
	•	Рибонуклеопротеиновые комплексы участвуют в регуляции повреждённого эндотелия и активации воспалительных каскадов.

**“positive regulation of DNA metabolic process”, “positive regulation of DNA repair”**
	•	Операция и реперфузия могут вызывать оксидативный стресс, повреждения ДНК и активацию механизмов репарации.
	•	ДНК-ремонтные пути помогают предотвратить апоптоз клеток сердечной ткани.

**“chromosome segregation”, “sister chromatid segregation”, “nuclear chromosome segregation”, “spindle organization”**
	•	Возможно, связано с повышенной пролиферацией эндотелиальных клеток или иммунных клеток в ответ на воспаление после операции.
	•	Активация клеточного цикла может происходить в регенерирующих тканях.

**“mononuclear cell differentiation”, “cellular response to external stimulus”**
	•	Операция вызывает активацию иммунных клеток (моноциты, макрофаги), которые участвуют в заживлении.
	•	Стресс-индуцированные каскады влияют на экспрессию иммунных генов и дифференцировку клеток.
```{r}
...
```
```{r}
GO_enrich_down150_bp@result$Description[1:20]
```
Даунрегуляция этих путей после аортокоронарного шунтирования (АКШ) может быть связана с общим стресс-ответом, метаболической адаптацией и подавлением нейронально-связанной активности в условиях воспаления и реперфузии.

**“synapse organization”, “axonogenesis”, “regulation of neuron projection development”, “positive regulation of nervous system development”, “neuron projection extension”, “positive regulation of neurogenesis”**
	•	Операция и гипоксия вызывают системный стресс, что может привести к снижению активности нейротрофических факторов (например, BDNF, NGF), регулирующих рост и развитие нейронов.
	•	Возможна дисфункция периферической нервной системы, связанная с воспалением и повреждением сосудов.
	•	АКШ может вызывать когнитивные изменения из-за микроинсультов, воспаления или влияния анестезии на нейрональные пути.
	
**“small GTPase-mediated signal transduction”, “regulation of small GTPase mediated signal transduction”** 
	•	Малые GTPases (Rho, Rac, Rab) участвуют в регуляции клеточной морфологии, транспорта и миграции.
	•	Их подавление может быть связано с нарушением внутриклеточного трафика и транспорта белков из-за стресса и воспаления.
	•	Снижение активности этих путей может указывать на дисфункцию цитоскелета и клеточного транспорта.

**“vesicle-mediated transport in synapse”, “synaptic vesicle cycle”, “vesicle organization”**
	•	Связано с подавлением синаптической передачи → снижение активности нервной системы.
	•	Может отражать сниженную потребность в нейрональной активности, поскольку организм приоритетно направляет ресурсы на восстановление сердечно-сосудистой системы.

**“protein localization to nucleus”, “positive regulation of protein localization”, “establishment of protein localization to organelle”**
	•	Подавление этих путей может указывать на сниженную ядерную активность в некоторых клетках (например, в нейронах или эндотелиальных клетках).
	•	Вероятно, связано с глобальным снижением скорости клеточного метаболизма в условиях стресса.

**“cell growth”, “developmental cell growth”, “developmental growth involved in morphogenesis”, “regulation of cell morphogenesis”**
	•	Организм приоритетно тратит энергию на восстановление сердечно-сосудистой системы, а не на активный клеточный рост.
	•	Может быть связано с подавлением ангиогенеза или пролиферации клеток в нецелевых тканях.

**“macroautophagy”**
	•	Аутофагия регулирует очистку клеток от повреждённых белков и органелл, но после операции её снижение может отражать изменение метаболических приоритетов.
	•	Возможна роль воспалительных сигналов, подавляющих аутофагию в некоторых клетках.
...
**Значимые результаты**
```{r}
signres_16 <- results(dds_16, contrast=c("condition", "after", "before"), alpha=0.05) 
summary(signres_16)
```
```{r}
signres_16 <- signres_16[which(signres_16$padj < 0.05), ]
```

```{r}
gene_idx <- which.min(res_16$padj)
gene_name <- rownames(res_16)[gene_idx]  
...
count_data <- plotCounts(dds_16, gene=gene_idx, intgroup="condition", returnData=TRUE)
count_data$sample <- gsub("_.*", "", rownames(count_data))  
...
plt
ggsave("./pictures/boxplots_humoral_hsa-miR-126-3p.tiff", plot = plt, width = 16, height = 10, dpi = 300)
```

Let's arranged it by  padj
```{r}
order_indices <- order(res_16$padj)
...
plt <- EnhancedVolcano(res_16,
                ...
                title = NULL,        
                subtitle = NULL,
                selectLab = rownames(signres_16),  # 👈 все метки будут отображены
                drawConnectors = TRUE               # стрелочки, если метки накладываются
) 
...
```{r}
converted_mirna_down16 <- miRNAVersionConvert(down_16)
converted_mirna_down16
```
**Запрос таргетов из базы multiMiR**
```{r}
targets16_up <- unique(get_multimir(org = "hsa", mirna = converted_mirna_down16$Accession, table = "validated")@data$target_symbol)
```
...
```{r}
plt <- dotplot(GO_enrich_up16_bp, showCategory = 20) + 
  ...
  scale_y_discrete(labels = function(x) str_wrap(x, width = 40))
plt
ggsave("./pictures/GO_enrichment_dotplot_BP_type16.tiff", plot = plt, width = 16, height = 10, dpi = 300)
```
...
```
```{r}
GO_enrich_up16_bp@result$Description[1:20]
```
...
plt <- dotplot(GO_enrich_up16_MF, showCategory = 20) + 
...
plt
ggsave("./pictures/GO_enrichment_dotplot_MF_type16.tiff", plot = plt, width = 16, height = 10, dpi = 300)
```
...
# collapsed_df <- collapsed_df[, rownames(coldata)] #ранжирую по колонки в counts так же как и названия строк в coldata
head(collapsed_df)
```
```{r}
dds <- DESeqDataSetFromMatrix(countData = collapsed_dfs, 
                              colData = coldata, 
                              design = ~type*condition + patient)
dds$condition <- relevel(dds$condition, ref = "before")
dds
