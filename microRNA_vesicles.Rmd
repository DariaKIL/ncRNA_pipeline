...
mirna_names_down <- c("hsa-miR-133a-3p", "hsa-miR-208b-3p", "hsa-miR-499a-5p")
```
...
```
После АКШ организм пациента переходит в фазу адаптации и восстановления, что отражается в регуляции биологических процессов:
	•	Активируется транспорт белков, аутофагия, регенерация тканей и пластичность нервной системы, что помогает клеткам выжить и восстановиться.
	•	Снижается активность клеточного деления, экспрессии генов и иммунного ответа, что может предотвращать воспалительное повреждение и поддерживать гомеостаз.
```{r}
...
```{r}
converted_mirna_up16 <- miRNAVersionConvert(up_16)
converted_mirna_up16
```
**Запрос таргетов из базы multiMiR**
```{r}
targets16_down <- unique(get_multimir(org = "hsa", mirna = converted_mirna_up16$Accession, table = "validated")@data$target_symbol)
```
...
```{r}
plt <- dotplot(GO_enrich_down16_bp, showCategory = 20) + 
...
```{r}
GO_enrich_DOWN16_BP <- enrichplot::pairwise_termsim(GO_enrich_down16_bp, method = "JC")

plt <- emapplot(GO_enrich_DOWN16_BP, 
         repel = TRUE,
         showCategory = 20) +
...
plt <- dotplot(GO_enrich_down16_MF, showCategory = 20) + 
