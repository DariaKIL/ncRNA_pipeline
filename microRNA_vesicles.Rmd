...
coldata$new_col <- paste0(coldata$condition, "_type", coldata$type)
...
# anno_long <- summed_anno %>%
#   rowwise() %>%
#   mutate(across(-new_col, ~ . / sum(c_across(-new_col)))) %>% 
#   ungroup() %>%
#   pivot_longer(cols = -new_col, names_to = "RNA_Type", values_to = "Proportion")

plt <- ggplot(anno_long, aes(x = new_col, y = Count, fill = RNA_Type)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
